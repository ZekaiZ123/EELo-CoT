...
class DeepSeekQwenModel(Qwen2ForCausalLM):
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        """
        # Initialize injection-related variables
        self.wait_token_prob = 0.0
        self.seen_token_ids = set()
        self.wait_token_alpha = 0.6  # you can tune this
        self.token_count = 0
...
# ==========================================================
            ...
            # Innovation-based wait token probability update
            last_token_id = next_tokens[0].item()
            self.token_count += 1
            
            # Default increment per token
            self.wait_token_prob += 0.01  # small steady increase
            
            # Innovation-based boost
            if last_token_id not in self.seen_token_ids:
                self.seen_token_ids.add(last_token_id)
                token_str = self.tokenizer.decode([last_token_id], skip_special_tokens=True)
                if any(char.isdigit() for char in token_str):
                    self.wait_token_prob += self.wait_token_alpha * 1.0
            else:
                self.wait_token_prob += 0.01  # minor bump for repeat tokens
                
            # Determine if sentence boundary is reached
            full_text = self.tokenizer.decode(input_ids[0], skip_special_tokens=False)
            all_ids = input_ids[0].tolist()
            is_sentence_end, self.last_period_idx = self.check_trigger_from_last_period(
                    token_ids=all_ids,
                    min_digits=self.min_digits
            )
            
            
            # Decide whether to inject "Wait"
            inject_wait = self.wait_token_prob > 1.0 and is_sentence_end
            if inject_wait:
                cur_idx = len(input_ids[0])
                if cur_idx > self.last_injected_idx:
                    self.last_injected_idx = cur_idx
                    self.wait_token_prob = 0.0  # reset after insertion
                    ...
                    forced_attn = forced_out.attentions if not self.config.is_encoder_decoder else forced_out.decoder_attentions
                    forced_hs = forced_out.hidden_states if not self.config.is_encoder_decoder else forced_out.decoder_hidden_states
